...
export const customHandleColorChange = (
  data: { el: string; quantity: number },
  setProductData: (prevState: any) => any,
  i?: number
) => {
  setProductData((prevData: any) => {
    const colors = [...(prevData.colors || [])];
    const customData = { color: data.el, quantity: data.quantity };
    if (i !== undefined) {
      if (
        data.el === "" ||
        colors.some((e, k) => e.color === data.el && k !== i)
      ) {
        colors.splice(i, 1);
      } else {
        colors[i] = customData;
...
export const customHandleSizeChange = (
  ...
  setProductData((prevData: any) => {
    ...
    const customData = { size: data.el, quantity: data.quantity };
    if (i !== undefined) {
      if (
        data.el === "" ||
        sizes.some((e, k) => e.size === data.el && k !== i)
      ) {
        sizes.splice(i, 1);
      } else {
        sizes[i] = customData;
